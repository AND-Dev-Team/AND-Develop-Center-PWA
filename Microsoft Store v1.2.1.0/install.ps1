...
Write-Host "Installing AND Develop Center..."
$currentDir = Get-Location
$msixPath = ($currentDir.Path + "\" + "AND Develop Center.sideload.msix")
...
if ($installProc.ExitCode -eq 0) {
	$app = Get-StartApps "AND Develop Center"

	# If it's an array, then we found multiple matching apps. Grab the last one.
	if ($app -is [array]) {
		Write-Host "Warning: found multiple apps installed named AND Develop Center. Launching best guess. If the wrong app launches, find the right one in your start menu."
		$app = $app[-1];
	}

	if ($app) {
		Write-Host "Launching AND Develop Center..."
